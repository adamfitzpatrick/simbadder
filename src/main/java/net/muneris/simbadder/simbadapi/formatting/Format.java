...
/**
 * The Format class allows for simplified generation of an appropriate formatting
 * string to pass to the SIMBAD api along with the desired query.  Utilizing this
 * class eliminates the difficulty of ensuring that a formatting string contains
 * the proper SIMBAD value parameters, URI encoding and textual elements to return
 * a machine readable JSON string.
 * 
 * Format is primarily intended to accept items from the FormatField enum in order
 * desired response fields.  As an alternative, it may be configured to accept
 * format fields as well, although this is not recommended as it defeats the purpose
 * of the class.
 * 
...
public class Format {
	...
	private final static String FORMAT_PREFIX = "format object %s \"{";
	/** Bracket to close JSON formatted return data. */
	private final static String FORMAT_SUFFIX = "}\"";
	 ...
	 * must be named.  Although this capability is not currently implemented
	 * in this microservice, the format string name is included for a future
	 * use case.
	 ...
	 * @param provides a pre-configured formatting string
	 */
	 ...
	 * @param field specifier to be removed
	 */
	...
	public String getFormatString() {
		String format =fields.entrySet().stream()
				.map(fieldEntry -> fieldEntry.getValue())
