 ...
 *
 * Although it is not possible to specify a more granular level of formatting
 * options with this object, each top level formatting element will incorporate
 * all data that is available for that set.
 *
 * For example, specifying the coordinate (COO) format item means that the data
 * returned in the COO object will also contain right ascension (ra),
 * declination (dec), wavelength, quality code, error ellipse, and bibcode.
 *
 ...
 * parsing it was necessary at times to use property names designed around
 * avoiding option specifiers. Refer especially to FLUXLIST.
 *
...
public enum FormatField {
    ...
    MAINOTYPE("mainOType: { %OTYPE(numeric: !`!N!`!, shortname: !`!S!`!,"
            + " veryshortname: !`!3!`!, verbose: !`!V!`!) }"),
     ...
     * coordinates or an object ID. Requesting that information from a search
     * that cannot provide it provokes an error response from SIMBAD.
     *
     * @return list of formats excluding FormatField.DIST
     */
    public static List<FormatField> getAllNotDist() {
        return getAll().stream().filter(p -> !p.equals(FormatField.DIST))
    ...
    /**
     * Returns the formatting string for this object that will be passed to
     * SIMBAD.
     *
